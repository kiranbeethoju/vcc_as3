...
log "COMMAND" "gcloud compute instance-groups managed list-instances auto-scale-group --zone asia-south2-b"
INSTANCE_NAME=$(gcloud compute instance-groups managed list-instances auto-scale-group --zone asia-south2-b --format="value(name)" 2>/dev/null | head -n 1)
...
if (( $(echo "$CPU_USAGE > $CPU_THRESHOLD" | bc -l) )); then
    if [ -z "$INSTANCE_NAME" ]; then
        ...
        log "COMMAND" "Creating instance template: auto-scale-group"
        if gcloud compute instance-templates create auto-scale-group \
        ...
        log "COMMAND" "Creating managed instance group: auto-scale-group"
        if gcloud compute instance-groups managed create auto-scale-group \
            --base-instance-name scaled-vm \
            --template auto-scale-group \
            --size 1 \
        ...
        log "COMMAND" "Setting autoscaling for auto-scale-group"
        if gcloud compute instance-groups managed set-autoscaling auto-scale-group \
            --max-num-replicas 3 \
            --min-num-replicas 1 \
        ...
        GCP_VM_NAME=$(gcloud compute instance-groups managed list-instances auto-scale-group --zone asia-south2-b --format="value(name)" | head -n 1)
...
# If CPU usage drops below threshold and VM exists, shut down the VM
    ...
    if [ -n "$INSTANCE_NAME" ]; then
        ...
        log "COMMAND" "gcloud compute instance-groups managed delete auto-scale-group --zone asia-south2-b"
       
        if gcloud compute instance-groups managed delete auto-scale-group --zone asia-south2-b --quiet; then
